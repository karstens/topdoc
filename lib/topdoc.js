  ...
  var Topdoc = (function() {
    ...
    Topdoc.prototype.parseFiles = function() {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        file = _ref[_i];
        var rawCss = read(file, 'utf8');
        var data = this.parseFileData(file);
        var topdocParse = new TopdocParse(rawCss, data);
        results.push(topdocParse.results);
      }
      return results;
    };
    Topdoc.prototype.parseFileData = function(file) {
      var data = {};
      var filename = path.basename(file);
      var basename = filename.substring(0, filename.length - path.extname(filename).length);
      var name_array = basename.split('-');
      data = {
